...
TESTS = dict(
    # People
    org=dict(description="Missing home organization, NeIC area or role.",
        callback=test_missing, kw=dict(path='people', blame='{obj_id}', specs=dict(
            ...
            area="Missing NeIC area.",
            ))),
    transit=dict(description="Missing arrivals, departures or shuttle requests.",
        callback=test_missing, args=('people', '{obj_id}', dict(
            arrival="Missing arrival day.",
            departure="Missing departure day.",
            shuttle="Missing airport shuttle request.",
            ))),
    size=dict(description="Missing corpulence.",
        callback=test_missing, args=('people', '{obj_id}', dict(
            size="Missing size.",
            ))),
    # Sessions
    chair=dict(description="Missing chairs.",
        callback=test_missing, args=('sessions', None, 'chair')),
    plenary=dict(description="Missing plenary y/n.",
        callback=test_missing, args=('sessions', None, dict(
            plenary="Missing plenary yes/no specification in sessions[{obj_id!r}] {obj[title]}.",
            ))),
    session_title=dict(description="Missing session titles.",
        callback=test_missing, args=('sessions', '{obj[chair]}', 'title')),
    session_abstract=dict(description="Missing session abstracts.",
        callback=test_missing, args=('sessions', '{obj[chair]}', 'abstract', True)),
    room=dict(description="Missing rooms.",
        callback=test_missing, args=('sessions', None, 'room')),
    # Talks
    speaker=dict(description="Missing speakers.",
        callback=test_talk_missing, args=('speaker',)),
    talk_title=dict(description="Missing talk titles.",
        callback=test_talk_missing, args=('title',)),
    talk_abstract=dict(description="Missing abstracts.",
        callback=test_talk_missing, args=('abstract', True)),
    slides=dict(description="Missing slides.",
        callback=test_talk_missing, args=('slides',)),
    )

_PERSONAL_TESTS = ['org', 'transit', 'size', 'session_title', 'session_abstract', 'speaker', 'talk_title', 'talk_abstract', 'slides']
_GENERAL_TESTS = ['chair', 'plenary', 'room']
...
METATESTS = dict(
    ...
    general=dict(description="General complaints; short for %s." % ','.join(sorted(_GENERAL_TESTS)), 
        tests=_GENERAL_TESTS),
    )
